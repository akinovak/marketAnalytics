...
class MojAutoScrap(scrapy.Spider):
    ...
    start_urls = [
        'https://www.mojauto.rs/rezultat/status/automobili/vozilo_je/polovan/poredjaj-po/oglas_najnoviji/po_stranici/20/prikazi_kao/lista/stranica/1']
    cars = []
    ...
    def parse(self, response):
        ...
        # print (tmpStr)
        numE = int(re.search('Prikazano 20 od ([0-9]*).([0-9]*)', tmpStr).group(1)) * 1000 + int(
            re.search('Prikazano 20 od ([0-9]*).([0-9]*)', tmpStr).group(2) if re.search(
                'Prikazano 20 od ([0-9]*).([0-9]*)', tmpStr).group(2) != '' else 0)
        # print ('OGLASA: ' + str(numE))
        numPages = int(math.ceil(numE / 20)) + 1
        # print ('BROJ STRNICA: ' + str(numPages))
        for i in range(numPages):
            url = 'https://www.mojauto.rs/rezultat/status/automobili/vozilo_je/polovan/poredjaj-po/oglas_najnoviji/po_stranici/20/prikazi_kao/lista/stranica/' + str(
                i + 1)
            arr_urls.append(url)
